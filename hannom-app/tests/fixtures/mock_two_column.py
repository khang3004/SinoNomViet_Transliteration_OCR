...
Synthetic Vietnamese text-layer spans + Han OCR detections for two idealised
entries (#99, #100), used by the two_column dry-run and unit tests. Self-contained
(no runtime dependency) so the tests/dry-run need nothing else.
"""
...
PAGE_WIDTH = 600.0  # synthetic page width: Vietnamese column starts at x≈300
...
def mock_han_ocr() -> list[dict]:
    ...
    inside each entry's y-band so y-overlap pairing assigns them correctly. The
    watermark token is low-confidence non-CJK ⇒ dropped by the post-filter.
    """
    han = [
