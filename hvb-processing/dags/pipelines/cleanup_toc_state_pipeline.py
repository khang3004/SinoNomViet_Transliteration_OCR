...
with DAG(
    dag_id="hvb_cleanup_toc_state_pipeline",
    default_args=DEFAULT_ARGS,
    description=(
        "Delete stale page OCR/aligned + catalog/entries for selected pages; "
        "recreate Qdrant pairs collection. Run once before TOC re-OCR."
    ),
    schedule_interval=None,
    catchup=False,
    max_active_runs=1,
    tags=["hvb", "v2", "cleanup", "toc", "qdrant", "k8s"],
    params={
    ...
    build_hvb_k8s_pod_task(
        task_id="cleanup_toc_state",
        job_name="cleanup_toc_state",
