...
with DAG(
    dag_id="hvb_ocr_gemini_pages_pipeline",
    default_args=DEFAULT_ARGS,
    description="OCR Gemini từng trang — mỗi trang một JSON trên MinIO (isolated K8s pod)",
    schedule_interval=None,
    catchup=False,
    max_active_runs=1,
    tags=["hvb", "ocr", "gemini", "per-page", "k8s"],
    params={
        "doc_id": Param(
            default="hvb_base",
            type="string",
        ...
        "upload_minio": Param(
            default=True,
            type="boolean",
            description="Upload mỗi trang lên MinIO: hvb-ocr-result/ocr/gemini/{doc_id}/page_XXXX.json",
        ),
    },
) as dag:
    build_hvb_k8s_pod_task(
        task_id="run_gemini_page_loop",
        job_name="ocr_gemini_pages",
