...
BUCKET_RAW=$(get_config_val "minio" "bucket_raw")
RAW_PREFIX=$(get_config_val "minio" "raw_prefix")
RAW_DIR=$(get_config_val "paths" "raw_dir")
...
# Prefer project kubeconfig by default / Mặc định ưu tiên kubeconfig của project
...
cleanup() {
  # Cleanup port-forward process / Dọn dẹp tiến trình port-forward
  if [ -n "$PF_PID" ]; then
...
# Use port-forward by default (can disable via ALWAYS_FORWARD=0) / Mặc định dùng port-forward (có thể tắt bằng ALWAYS_FORWARD=0)
if [ "$ALWAYS_FORWARD" = "1" ] || echo "$MINIO_HOST" | grep -Eq "\.svc(\.|$)"; then
  ...
  if command -v kubectl &>/dev/null; then
    # Prefer explicit service first / Ưu tiên service chỉ định trước
    if kubectl get svc -n "$MINIO_NAMESPACE" "$MINIO_SERVICE" >/dev/null 2>&1; then
      ...
      # Fallback to auto-detect by name / Fallback tự dò theo tên service
      ...
      if [ -n "$SVC_LINE" ]; then
        ...
        echo "Try setting explicit env: MINIO_NAMESPACE=storage MINIO_SERVICE=minio"
        echo "Also verify kubeconfig/context: kubectl get svc -A | grep -i minio"
        exit 1
      fi
    fi
  elif echo "$MINIO_HOST" | grep -Eq "\.svc(\.|$)"; then
...
if [ ! -d "${ROOT}/${RAW_DIR}" ]; then
  echo "Missing local raw dir: ${ROOT}/${RAW_DIR}"
  exit 1
fi

echo "Uploading source PDFs from ${ROOT}/${RAW_DIR} -> ${BUCKET_RAW}/${RAW_PREFIX}/"
mc alias set local-minio "$MINIO_ENDPOINT" "$MINIO_ACCESS_KEY" "$MINIO_SECRET_KEY" --api s3v4 >/dev/null
mc mb "local-minio/${BUCKET_RAW}" 2>/dev/null || true
mc mirror --overwrite "${ROOT}/${RAW_DIR}/" "local-minio/${BUCKET_RAW}/${RAW_PREFIX}/"

echo "Done. Trigger DAG: hvb_pdf_split_pipeline"
